        ...
        window.onload = function () {
            ...
            if (currentUser == null) {
                ...
                function calculateTotalAmount(data) {
                    var totalAmount = 0;
                    for (var i = 0; i < data.length; i++) {
                        totalAmount += parseFloat(data[i]['Amount in Taka']);
                    }

                    // Display the total amount
                    var totalAmountSpan = document.getElementById('total-amount');
                    totalAmountSpan.textContent = totalAmount.toFixed(2); // Format the total amount as desired (e.g., with two decimal places)
                }
            }
        }